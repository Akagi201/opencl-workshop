...
__kernel void add_numbers(__global float4* data, __local float* local_result,
                          __global float* group_result) {
  float sum;
  float4 input1, input2,
      sum_vector;  // array of 4 floats which support vectorization
  ...
  sum_vector =
      input1 + input2;  // perform four floating-point additions simultaneously
