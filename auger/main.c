...
int main(int argc, char *argv[]) {
  ...
  error_check(err, "Couldn't create a context");
  program = build_program(context, device, PROGRAM_FILE);
  queue = clCreateCommandQueue(context, device, 0, &err);
  error_check(err, "Could not create a command queue");

  // RNG initialization
  clrngMrg31k3pStream *streams = clrngMrg31k3pCreateStreams(
      NULL, ntarget, &streamBufferSize, (clrngStatus *)&err);

  /* Create data buffer    */
  streams_d = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
  ...
  // err = clEnqueueWriteBuffer(queue, xa_d, CL_TRUE, 0, bytes, xa, 0, NULL,
  // NULL); err |= clEnqueueWriteBuffer(queue, ya_d, CL_TRUE, 0, bytes, ya, 0,
  // NULL, NULL);

  /* Kernel setup */
  kernel = clCreateKernel(program, KERNEL_FUNC, &err);
  error_check(err, "Could not create a kernel");
  ...
  err = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &globalsize, &localsize,
                               0, NULL, NULL);
  error_check(err, "Could not enqueue the kernel");
  clFinish(queue);
